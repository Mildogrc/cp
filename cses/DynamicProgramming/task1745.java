...
public class task1745 {
	public static void main(String[] args) throws IOException {
		...
		BitSet prev = new BitSet(mx);
		BitSet curr = new BitSet(mx);
		BitSet t;
		prev.set(0);
		for (int i = 0; i < N; i++, t = prev, prev = curr, curr = t)
			for (int j = 0; j < mx; j++)
				if (prev.get(j) || (j >= arr[i] && prev.get(j - arr[i])))
					curr.set(j);
		out.println(prev.cardinality() - 1);
		for (int i = 1; i < mx; i++)
			if (prev.get(i))
				out.print(i + " ");
		out.close();
	...
	static void swap(BitSet o1, BitSet o2) {
		BitSet t = o2;
		o2 = o1;
		o1 = t;
	}
