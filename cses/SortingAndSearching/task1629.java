...
public class task1629 {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < N; i++)
			list.add(mp(in.nextInt(), in.nextInt()));
		list.sort((a, b) -> Integer.compare(a.f, b.f));
		int[] dp = new int[N];
		int max = 0;
		Queue<Integer> q = new PriorityQueue<>((a, b) -> Integer.compare(list.get(a).s, list.get(b).s));
		for (int i = 0; i < N; i++) {
			while (!q.isEmpty() && list.get(q.peek()).s <= list.get(i).f)
