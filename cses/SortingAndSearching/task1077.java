...
public class task1077 {
	...
	static void BSTSolve(int N, int K, int[] arr) {
		...
		long oldm = findMedian();
		long d = 0;
		for (int i = 0; i < K; i++)
			d += Math.abs(arr[i] - oldm);
		sb.append(d + " ");
		for (int i = K; i < N; i++) {
			remove(arr[i - K]);
			addNum(arr[i]);
			long mid = findMedian();
			d = d + Math.abs(mid - arr[i]) - Math.abs(oldm - arr[i - K]);
			if (even)
				d -= (mid - oldm);
			oldm = mid;
			sb.append(d + " ");
	...
	static void BitSolve(int N, int K, int[] arr) {
		...
		for (int i = 0; i < K; i++)
			ost.add(arr[i]);
		long oldm = map[ost.getKth((K + 1) >> 1) - 1];
		long d = 0;
		for (int i = 0; i < K; i++)
			d += Math.abs(map[arr[i] - 1] - oldm);
		sb.append(d + " ");
		for (int i = K; i < N; i++) {
			ost.add(arr[i]);
			ost.remove(arr[i - K]);
			long mid = map[ost.getKth((K + 1) >> 1) - 1];
			d = d + Math.abs(mid - map[arr[i] - 1]) - Math.abs(oldm - map[arr[i - K] - 1]);
			if (even)
				d -= (mid - oldm);
			oldm = mid;
			sb.append(d + " ");
