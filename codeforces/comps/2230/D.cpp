...
void solve(int tc) {
    int n;
    cin >> n;
    vi a(n), b(n), nexta(n+5, n), nextb(n+5, n), dp(n+5, n-1), R(n, n); 
    F0R(i, n) cin >> a[i]; F0R(i, n) cin >> b[i];
    R0F (i, n) {
        nexta[a[i]] = i; nextb[b[i]] = i;
        if (a[i] == b[i]) {
            int na = nexta[a[i] + 1], nb = nextb[b[i] + 1];
            if (na == nb) dp[a[i]] = dp[a[i]+1];
            else dp[a[i]] = min(na, nb) - 1;
        }
        if (nexta[1] == nextb[1]) R[i] = dp[1];
        else R[i] = min(nexta[1], nextb[1]) - 1;
        R[i] = R[i] - i + 1;
    }
    long long sum = 0;
    F0R(L, n) sum += R[L];
